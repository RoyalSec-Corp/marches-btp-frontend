...
function ContratsList() {
  ...
  const loadContractMetrics = async (contractId) => { try { setLoadingMetrics(prev => ({ ...prev, [contractId]: true })); const metrics = await contractTrackingService.getRealTimeMetrics(contractId); setContractMetrics(prev => ({ ...prev, [contractId]: metrics })); } catch (err) { console.error('Erreur chargement metriques contrat', contractId, ':', err); } finally { setLoadingMetrics(prev => ({ ...prev, [contractId]: false })); } };
  ...
  return (
    <div className="bg-gradient-to-br from-blue-400 via-blue-500 to-blue-400 rounded-xl p-6 shadow-sm border border-blue-200">
      ...
      {filteredContrats.length === 0 ? (<div className="text-center py-8 text-gray-500">{contrats.length === 0 ? 'Aucun contrat trouve' : 'Aucun contrat ne correspond a votre recherche'}</div>) : (
        <div className="overflow-x-auto"><table className="min-w-full table-auto text-sm"><thead className="text-left text-white border-b"><tr><th className="py-2 px-3">Projet & Progression</th><th className="py-2 px-3">Freelance</th><th className="py-2 px-3">Dates & Metriques</th><th className="py-2 px-3">Montant</th><th className="py-2 px-3">Statut & Etape</th><th className="py-2 px-3">Actions</th></tr></thead>
          <tbody className="text-white">{filteredContrats.map((contrat) => (<tr key={contrat.id} className="border-b hover:bg-gradient-to-br from-blue-600 via-blue-700 to-blue-600 transition-colors duration-300">
