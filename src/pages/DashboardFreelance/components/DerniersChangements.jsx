  ...
  const loadRecentChanges = useCallback(async () => {
    try {
      ...
      setChangements([
        ...
        { type: 'paiement', icon: 'ri-money-euro-circle-line', title: 'Paiement recu', description: 'Mission electricite - 850 \u20ac', time: 'Il y a 3 jours', bg: 'bg-blue-50', iconBg: 'bg-blue-100', iconColor: 'text-blue-600' }
      ]);
    ...
    <div className="bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="space-y-4">
        {changements.map((item, index) => (
          <div key={index} className={`flex items-start space-x-3 p-3 rounded-lg bg-gradient-to-br from-orange-500/50 to-orange-500 bg-opacity-80 border border-orange-400/40 ${item.bg}`}>
            <div className={`w-8 h-8 ${item.iconBg} rounded-full flex items-center justify-center bg-gradient-to-br from-orange-400/50 to-orange-600 bg-opacity-50`}><i className={`${item.icon} ${item.iconColor}`}></i></div>
            <div><p className="text-sm font-medium text-white">{item.title}</p><p className="text-xs text-white">{item.description}</p><p className="text-xs text-white mt-1">{item.time}</p></div>
          </div>
        ))}
      </div>
    </div>
