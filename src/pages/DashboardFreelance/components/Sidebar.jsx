    ...
    <aside className={`fixed top-0 left-0 h-full w-64 bg-gradient-to-b from-blue-700 to-blue-800 border-r border-gray-200 z-40 transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-200 ease-in-out flex flex-col justify-between`}>
      ...
      <nav className="flex-1 px-4 space-y-2 mt-4">
        {menuItems.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => { setActive(id); if (onClose) onClose(); }} className={`flex items-center w-full px-4 py-3 rounded-lg font-medium text-sm transition-all ${active === id ? 'bg-blue-500 text-white' : 'text-white hover:text-white/80 hover:bg-blue-600'}`}><Icon className="w-5 h-5 mr-3" />{label}</button>
        ))}
